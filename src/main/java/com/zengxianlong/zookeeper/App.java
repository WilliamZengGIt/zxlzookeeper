package com.zengxianlong.zookeeper;
...
public class App 
{
    public static void main( String[] args ) throws IOException, InterruptedException, KeeperException {
        //zk是有session概念的没有连接池的概率。
        //watch：观察，回调
        //watch的注册只发生在读类型调用，如get、exitex。。。
        //第一类：new zk时候，传入的watch，这个watch，session级别的，跟path、node没有关系。
        CountDownLatch cd = new CountDownLatch(1);
        ZooKeeper zk = new ZooKeeper("192.168.152.128:2181,192.168.152.129:2181,192.168.152.130:2181,192.168.152.131:2181",
                3000,
                new Watcher() {
                    //Watch 的回调方法！
                    @Override
        ...
        switch (state) {
         ...
         String s = zk.create("/ooxx", "olddate".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
